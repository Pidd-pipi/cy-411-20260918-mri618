...
export const LogTemplates = {
  ...
  GOAL_ADJUSTMENT_START: 'Goal[id={id}] adjustment start effective_date={effectiveDate} new_value={newValue}',
  GOAL_ADJUSTMENT_SUCCESS: 'GoalAdjustment[id={id}] goal_id={goalId} created original_value={originalValue} new_value={newValue} effective_date={effectiveDate}',
  GOAL_ADJUSTMENT_FAILED: 'GoalAdjustment[goal_id={goalId}] failed: {field} {reason}',
  GOAL_ADJUSTMENT_PROGRESS_CALCULATED: 'GoalAdjustment[goal_id={goalId}] segment progress calculated segment={segment} emitted={emitted} cap={cap} remaining={remaining}',
